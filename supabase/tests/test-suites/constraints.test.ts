 ...
 *
 * Constraints Tested:
 * - UNIQUE: email (users), (category, value) (taxonomy), (user_id, recommended_user_id, algorithm_version) (user_match_cache)
 * - CHECK: role, url_type, category, entity_type, status, match_score (0-100), user_id != recommended_user_id
 * - FK: portfolio_company_id, taxonomy_id, user_id, user_match_cache.user_id, user_match_cache.recommended_user_id
 */
...
describe('user_match_cache Constraints', () => {
  describe('Foreign Key Constraints', () => {
    ...
    it('should reject invalid recommended_user_id', async () => {
      const { error } = await supabase.from('user_match_cache').insert({
        user_id: '00000000-0000-0000-0000-000000000000',
  ...
  describe('CHECK Constraints', () => {
    ...
    it('should allow valid match_score (0-100)', async () => {
      ...
      // Will error due to RLS (coordinator-only), but not due to CHECK constraint
      if (error) {
  ...
  describe('UNIQUE Constraint', () => {
    it('should enforce unique (user_id, recommended_user_id, algorithm_version)', async () => {
      ...
      const { error: error2 } = await supabase
        ...
        .insert({ ...matchData, match_score: 75 }); // Different score, same key

      // At least one should error (RLS or UNIQUE constraint)
      expect(error1 !== null || error2 !== null).toBe(true);
    });
  });
});
