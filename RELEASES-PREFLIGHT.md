...
- [ ] The previous release's bookkeeping is on `dev` (gate 0 fails when the version carriers and `CHANGELOG.md` as
      they stood at the anchor release never reached `dev`; run `scripts/sync-dev-after-release.sh v<version>` and
      rerun).
