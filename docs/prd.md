...
The CF Office Hours platform implementation is organized into **9 sequential epics** with **78 total stories**. Epics are ordered by dependency, with foundational infrastructure first and user-facing features building on top.
...
Epic 3: Profile Management
  ↓
Epic 4: Airtable Integration
  ↓
Epic 5: Calendar Integration
  ↓
Epic 6: Availability & Booking
  ↓
Epic 7: Matching & Discovery
  ↓
Epic 8: Reputation & Ratings
  ↓
Epic 9: Admin & Coordinator Tools
```
...
#### **Epic 1: Technical Foundation**
**Goal:** Establish database schema, API infrastructure, deployment pipeline, and core utilities
...
1. **DB-001: Database Schema Creation**
   - As a **developer**, I want the core database schema created so the application has a data foundation
   - **Acceptance Criteria:**
     - All tables from Section 4.1 created in Supabase
     - Constraints, indexes, and foreign keys implemented
     - Soft delete columns (`deleted_at`) on all relevant tables
     - Timestamp columns (`created_at`, `updated_at`) auto-populated
   - **Related:** FR82, NFR17, NFR18

2. **DB-002: Database Views for Soft Deletes**
   ...
   - **Acceptance Criteria:**
     ...
     - Application queries use views by default
   - **Related:** NFR29, NFR30

3. **DB-003: Row Level Security Policies**
   - As a **user**, I want my data protected so only authorized users can access it
   - **Acceptance Criteria:**
     - RLS policies implemented per Section 4.1
     - Users can only read/update their own profiles
     - Coordinators can access all data
     - Bookings visible only to participants + coordinators
   - **Related:** NFR12, Section 4.1

4. **API-001: Hono Framework Setup**
   - As a **developer**, I want a Hono-based API framework configured for Cloudflare Workers
   - **Acceptance Criteria:**
     - Hono app initialized with routing structure from Section 4.2
     - Middleware stack: auth, error handling, CORS
     - OpenAPI documentation generation via `@hono/zod-openapi`
     - Health check endpoint (`/api/health`) functional
   - **Related:** FR76, FR77, FR78, NFR44

5. **API-002: Centralized Error Handling**
...
6. **API-003: Authentication Middleware**
   - As a **developer**, I want reusable auth middleware for protecting endpoints
   - **Acceptance Criteria:**
     - `requireAuth` middleware verifies Supabase JWT tokens
     - `requireRole` middleware checks user roles
     - User context injected into request context (`c.set('user', user)`)
     - 401 errors for missing/invalid tokens, 403 for insufficient permissions
   - **Related:** FR100, NFR11

7. **API-004: Zod Schemas & OpenAPI Generation**
...
8. **UTIL-001: Date/Time Utilities**
...
9. **UTIL-002: Database Query Helpers**
...
10. **DEPLOY-001: Cloudflare Workers Deployment**
    - As a **developer**, I want the API deployed to Cloudflare Workers
    - **Acceptance Criteria:**
      - `wrangler.toml` configured per Section 4.6
      - Environment secrets configured via Wrangler CLI
      - Production deployment accessible at API domain
      - Health check endpoint returns 200 OK
    - **Related:** Section 4.6, NFR44

11. **DEPLOY-002: Cloudflare Pages Deployment**
...
12. **TEST-001: Testing Infrastructure Setup**
...
#### **Epic 2: Authentication & User Management**
**Goal:** Enable user authentication, role management, and basic user CRUD
**Priority:** P0 (Blocking)
**Estimated Stories:** 8
**Dependencies:** Epic 1
...
13. **AUTH-001: Magic Link Authentication**
    - As a **user**, I want to log in via passwordless magic link sent to my email
    - **Acceptance Criteria:**
      ...
      - Email must exist in synced Airtable users table (FR3)
      - Error message: "Contact admin for access" if email not found (FR4)
    - **Related:** FR1, FR3, FR4, Section 3.3

14. **AUTH-002: Google OAuth Authentication (Combined Auth + Calendar)**
...
15. **AUTH-003: Microsoft OAuth Authentication (Combined Auth + Calendar)**
...
16. **AUTH-004: Email Whitelist Validation**
    - As a **developer**, I want to restrict authentication to Airtable-synced users
    - **Acceptance Criteria:**
...
17. **USER-001: User Profile API Endpoints**
    - As a **developer**, I want API endpoints for user profile CRUD operations
    - **Acceptance Criteria:**
      - `GET /api/users/me` returns current user + profile
      - `PUT /api/users/me` updates current user profile
      - `GET /api/users/:id` returns user profile (public data)
      - `GET /api/users/search` supports filtering by role, tags, status
    - **Related:** FR7, Section 4.2

18. **USER-002: User Profile View/Edit Screen**
    - As a **user**, I want to view and edit my profile information
    - **Acceptance Criteria:**
      - Profile page shows: avatar, name, title, company, contact info, tags
      - Edit mode: inline editing with save/cancel
      - Form validation: required fields, email format, URL format
      - Toast notification on successful save
    - **Related:** FR7, Section 3.3

19. **USER-003: Role-Based Access Control**
    - As a **developer**, I want users assigned roles (mentee, mentor, coordinator) with appropriate permissions
    - **Acceptance Criteria:**
      - User role stored in `users.role` (enum)
      - API middleware validates role for protected endpoints
      - RLS policies enforce role-based data access
      - Frontend routes restricted by role
    - **Related:** FR6, NFR12

20. **USER-004: User Search & Directory**
    - As a **user**, I want to search for other users by name, company, or tags
    - **Acceptance Criteria:**
      - Search input with autocomplete
      - Filters: role, tags, company, active/inactive
...
#### **Epic 3: Profile Management**
**Goal:** Enable users to create rich profiles with tags, documents, and avatars
**Priority:** P0 (Blocking)
**Estimated Stories:** 6
**Dependencies:** Epic 2
...
21. **PROFILE-001: Tag Selection UI**
    ...
    - **Acceptance Criteria:**
      ...
      - "Add custom tag" option triggers new tag request workflow
    - **Related:** FR11, FR12, FR91, FR92

22. **PROFILE-002: Mentee-Specific Profile Fields**
...
23. **PROFILE-003: Mentor-Specific Profile Fields**
...
24. **PROFILE-004: Avatar Upload & Cropping**
...
25. **PROFILE-005: Additional Links Management**
...
26. **PROFILE-006: Reminder Preferences**
...
#### **Epic 4: Airtable Integration**
**Goal:** Sync user data and taxonomy from Airtable to Supabase
**Priority:** P0 (Blocking)
**Estimated Stories:** 5
**Dependencies:** Epic 1
...
27. **AIRTABLE-001: Webhook Endpoint Setup**
...
28. **AIRTABLE-002: Users Table Sync**
...
29. **AIRTABLE-003: User Tags Sync**
...
31. **AIRTABLE-005: User Deletion Handling**
...
#### **Epic 5: Calendar Integration**
**Goal:** Integrate Google Calendar and Microsoft Outlook for availability checking and event creation
**Priority:** P0 (Blocking)
...
32. **CAL-001: ICalendarProvider Interface Definition**
...
33. **CAL-002: Google Calendar Provider Implementation**
    ...
    - **Acceptance Criteria:**
      ...
      - Implements all interface methods from CAL-001
    - **Related:** FR21, NFR15, CR2, Section 4.5

34. **CAL-003: Microsoft Outlook Provider Implementation**
...
35. **CAL-004: Post-Login Calendar Connection (Magic Link Users)**
...
36. **CAL-005: Calendar Connection Requirement Enforcement**
...
37. **CAL-006: Calendar Conflict Checking**
...
38. **CAL-007: Calendar Event Creation**
...
39. **CAL-008: Google Meet Link Generation**
...
40. **CAL-009: Calendar Disconnection**
...
41. **CAL-010: iCal Feed for Calendar Subscription**
...
#### **Epic 6: Availability & Booking**
**Goal:** Enable mentors to set availability and mentees to book time slots
**Priority:** P0 (Blocking)
...
42. **AVAIL-001: Create Availability Block (API)**
    - As a **developer**, I want an API endpoint for mentors to create availability blocks
    - **Acceptance Criteria:**
      - `POST /api/availability` endpoint accepts CreateAvailabilitySchema (Section 4.2)
      - Validates: recurrence pattern, date range, time range, slot duration, meeting type
      - Generates `time_slots` rows based on recurrence pattern
      - Prevents overlapping availability blocks (validation error)
      - Returns created availability block with generated slot count
    - **Related:** FR22, FR23, FR24, FR25, Section 4.2

43. **AVAIL-002: Availability Management Dashboard**
    - As a **mentor**, I want to view and manage my availability blocks
    - **Acceptance Criteria:**
      - Dashboard shows list/cards of availability blocks
      - Each card: recurrence pattern, time range, slot duration, meeting type, booking count
      - Actions: Edit, Delete (blocked if slots booked with warning)
      - "Create Availability" button opens form
    - **Related:** FR22, FR80, Section 3.3

44. **AVAIL-003: Availability Block Form**
    - As a **mentor**, I want to create availability blocks with flexible recurrence patterns
    - **Acceptance Criteria:**
      - Form fields: recurrence (one-time/weekly/monthly/quarterly), date range, time range, slot duration (15/20/30/60 min), buffer time, meeting type
      - Meeting type options: in-person (preset location dropdown), in-person (custom location input), online (auto Google Meet)
      - Preview: "This will create X slots per week"
      - Validation: end time > start time, valid date range, buffer time < slot duration
    - **Related:** FR22, FR23, FR24, FR25, Section 3.3

45. **AVAIL-004: Time Slot Generation Logic**
...
46. **AVAIL-005: Slot Picker UI Component**
...
47. **AVAIL-006: Real-Time Slot Availability Updates**
...
48. **BOOK-001: Booking Creation (API)**
    - As a **developer**, I want an API endpoint to create bookings with validation
    - **Acceptance Criteria:**
      - `POST /api/bookings` endpoint accepts CreateBookingSchema
      - Validation: slot not already booked, meets 1-day advance requirement, no calendar conflicts
      - Database transaction: Mark `time_slots.is_booked=true`, create `bookings` record
      - Optimistic locking: 409 error if slot taken during processing (race condition)
      - Calls calendar provider to create events for both parties
    - **Related:** FR29, FR30, FR36, FR38, NFR7, NFR18

49. **BOOK-002: Booking Form UI**
    - As a **mentee**, I want to provide meeting context when booking a slot
    - **Acceptance Criteria:**
      - Form appears after slot selection
      - Required: Meeting goal (textarea, min 10 chars)
      - Optional: Materials URLs (multiple inputs)
      - Confirmation message: "This meeting is scheduled for [DATE/TIME] in your timezone ([TIMEZONE])"
      - Actions: "Confirm Booking" (primary), "Cancel" (returns to slot picker)
    - **Related:** FR30, Section 3.3

50. **BOOK-003: Booking Confirmation & Notifications**
    - As a **user**, I want to receive confirmation when a booking is created
    - **Acceptance Criteria:**
      - On booking success: Toast notification "Meeting booked! Calendar invite sent."
      - Email notification sent to both parties (INotificationProvider.sendBookingConfirmation)
      - Email includes: meeting details, Google Meet link (if applicable), calendar invite (.ics)
      - Notification logged in `notification_log`
    - **Related:** FR32, FR36, FR62, NFR27

51. **BOOK-004: My Bookings Dashboard**
    - As a **user**, I want to view all my upcoming, past, and canceled meetings
    - **Acceptance Criteria:**
      - Tabs: Upcoming / Past / Canceled
      - Meeting cards: participant info, date/time, meeting type, location/link, meeting goal
      - Upcoming: Actions = Cancel meeting, Copy meeting link, Add to calendar
      - Past: "Rate this session" if not yet rated
      - Canceled: Grayed out with cancellation reason/date
    - **Related:** Section 3.3

52. **BOOK-005: Meeting Cancellation**
    ...
    - **Acceptance Criteria:**
      ...
      - Email notification sent to other party (INotificationProvider.sendCancellation)
      - Canceled <2 hours before start: Reputation penalty applied (FR60)
    - **Related:** FR34, FR60, FR61

53. **BOOK-006: Meeting Reminders**
    - As a **user**, I want reminder emails before my meetings based on my preferences
    - **Acceptance Criteria:**
      - Background job: Check upcoming meetings and user reminder preferences
...
54. **BOOK-007: Preset Location Management**
...
55. **MATCH-001: IMatchingEngine Interface Definition**
...
56. **MATCH-002: Tag-Based Matching Algorithm (MVP)**
...
57. **MATCH-003: Recommended Mentors API**
    - As a **mentee**, I want personalized mentor recommendations based on my profile
    - **Acceptance Criteria:**
      - `GET /api/mentors/:id/recommended?menteeId={uuid}` endpoint
...
58. **MATCH-004: Recommended Mentees API**
    - As a **mentor**, I want personalized mentee recommendations based on my expertise
    - **Acceptance Criteria:**
      - `GET /api/mentees?recommended=true` endpoint
...
59. **MATCH-005: Mentor Directory UI**
    - As a **mentee**, I want to browse all mentors with filtering and search
    - **Acceptance Criteria:**
...
60. **MATCH-006: Mentee Directory UI**
    - As a **mentor**, I want to browse all mentees with filtering and search
    - **Acceptance Criteria:**
      - Similar layout to mentor directory
...
61. **MATCH-007: Mentor Send Interest (Reach Out)**
...
62. **REP-001: IReputationCalculator Interface Definition**
...
63. **REP-002: Reputation Score Calculation Logic**
...
64. **REP-003: Reputation Tier Assignment**
...
65. **REP-004: Tier-Based Booking Limits**
...
66. **REP-005: Tier Restriction on Mentor Booking**
    ...
    - **Acceptance Criteria:**
      ...
      - Exception request workflow in REP-008
    - **Related:** FR51, FR71

67. **REP-006: Post-Meeting Rating Prompt**
...
68. **REP-007: Reputation Score Display**
...
69. **REP-008: Tier Override Request (Mentee-Initiated)**
...
70. **REP-009: Dormant User Detection**
...
71. **REP-010: Reputation History Tracking**
...
72. **ADMIN-001: Dashboard KPI Widgets**
...
73. **ADMIN-002: Tier Override Request Management**
...
74. **ADMIN-003: Email Approve Link (Magic Link)**
...
75. **ADMIN-004: Manual Reputation Override**
...
76. **ADMIN-005: White-Glove Scheduling**
...
77. **ADMIN-006: Meeting Management**
...
78. **ADMIN-007: Tag Approval Workflow**
...
**Total Stories:** 78
**Critical Path (P0):** Epics 1-6 (54 stories)
**High Priority (P1):** Epics 7-8 (17 stories)
**Medium Priority (P2):** Epic 9 (9 stories)

**Recommended Sprint Breakdown (2-week sprints):**

- **Sprint 1:** Epic 1 (Technical Foundation) - 12 stories
- **Sprint 2:** Epic 2 (Auth & User Management) - 8 stories
- **Sprint 3:** Epic 3 (Profile Management) + Epic 4 (Airtable Integration) - 11 stories
- **Sprint 4:** Epic 5 (Calendar Integration) - 10 stories
- **Sprint 5-6:** Epic 6 (Availability & Booking) - 13 stories
- **Sprint 7:** Epic 7 (Matching & Discovery) - 7 stories
- **Sprint 8:** Epic 8 (Reputation & Ratings) - 10 stories
- **Sprint 9:** Epic 9 (Admin & Coordinator Tools) - 9 stories

**Estimated Timeline:** 18 weeks (9 sprints)
...
- All API endpoints implement centralized error handling (API-002)
...
Epic 1 (Technical Foundation)
  └─> Epic 2 (Auth & User Management)
        └─> Epic 3 (Profile Management)
              └─> Epic 4 (Airtable Integration) [parallel to Epic 5]
              └─> Epic 5 (Calendar Integration)
                    └─> Epic 6 (Availability & Booking)
                          └─> Epic 7 (Matching & Discovery) [parallel to Epic 8]
                          └─> Epic 8 (Reputation & Ratings)
                                └─> Epic 9 (Admin & Coordinator Tools)
```

**Critical Dependencies:**
- Calendar Integration (Epic 5) must complete before Availability & Booking (Epic 6)
- Availability & Booking (Epic 6) must complete before Reputation (Epic 8)
- Profile Management (Epic 3) must complete before Matching (Epic 7)

**Parallelization Opportunities:**
- Airtable Integration (Epic 4) can run parallel to Calendar Integration (Epic 5) after Epic 3 completes
- Matching & Discovery (Epic 7) can run parallel to Reputation & Ratings (Epic 8) after Epic 6 completes
