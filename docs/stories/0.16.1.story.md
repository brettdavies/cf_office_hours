...
- [ ] **Task 1: Add console logging to auth flow** (AC: 1, 7)
  - [ ] `apps/web/src/hooks/useAuth.ts`: Add logs for session events, profile fetch
  - [ ] `apps/web/src/pages/auth/LoginPage.tsx`: Add logs for magic link send (already present, verify format)
  - [ ] `apps/web/src/pages/auth/CallbackPage.tsx`: Add logs for redirect logic, race condition detection
  - [ ] Ensure logging format: `[AUTH] Action { context }`

- [ ] **Task 2: Fix magic link redirect bug** (AC: 7 - BLOCKER)
  - [ ] Add `isInitializing` state to `useAuth` hook to track if `getSession()` has completed
  - [ ] Update `isLoading` logic: `isLoading: isInitializing || (session !== null && user === null)`
  - [ ] Add detailed logging to `CallbackPage` redirect decision logic
  - [ ] Add warning log if premature redirect detected: `console.warn('[AUTH] Premature redirect prevented - auth params present but session not yet established')`
  - [ ] Test locally with magic link flow before committing

- [ ] **Task 3: Add console logging to booking flow** (AC: 2)
  - [ ] `apps/web/src/pages/mentors/MentorProfilePage.tsx` (or wherever slot picker is): Log slot selection
  - [ ] Booking form component: Log form submission, API call
  - [ ] `apps/api/src/routes/bookings.ts`: Log booking creation attempts, success/failure
  - [ ] `apps/api/src/services/booking.service.ts`: Log service-level booking logic
  - [ ] Ensure logging format: `[BOOKING] Action { context }`

- [ ] **Task 4: Add console logging to availability flow** (AC: 3)
  - [ ] `apps/web/src/pages/availability/AvailabilityPage.tsx`: Log availability form submission
  - [ ] `apps/api/src/routes/availability.ts`: Log availability creation requests
  - [ ] `apps/api/src/services/availability.service.ts`: Log slot generation count
  - [ ] Ensure logging format: `[AVAILABILITY] Action { context }`

- [ ] **Task 5: Create API request logging middleware** (AC: 4)
  - [ ] Create `apps/api/src/middleware/logging.ts`
  - [ ] Middleware logs: method, endpoint, userId (from auth context), timestamp, status code
  - [ ] Wire into `apps/api/src/index.ts` (apply before routes)
  - [ ] Ensure logging format: `[API] ${method} ${endpoint} { userId, statusCode, timestamp }`

- [ ] **Task 6: Create manual test checklist template** (AC: 6)
  - [ ] Create `docs/qa/0.16.1-manual-test-checklist.md`
  - [ ] Template structure: Test case name, Steps, Expected result, Actual result, Status (PASS/FAIL)
  - [ ] Include test cases listed below in "Manual Test Checklist" section

- [ ] **Task 7: Create gap documentation template** (AC: 6)
  - [ ] Create `docs/qa/0.16.1-manual-testing-results.md`
  - [ ] Template sections: Summary, BLOCKER Gaps, IMPORTANT Gaps, MINOR Gaps, Notes
  - [ ] BLOCKER definition: Prevents deployment or breaks critical flow
  - [ ] IMPORTANT definition: Major issue but workaround exists
  - [ ] MINOR definition: Minor UX issue or edge case

- [ ] **Task 8: Run automated tests to verify no regressions** (AC: 1-5)
  - [ ] Run `npm run test:api` - all tests must pass
  - [ ] Run `npm run test:web` - all tests must pass
  - [ ] Run `npm run lint` - no errors
  - [ ] Verify console logs appear in test output (check DEV mode logs present)

- [ ] **Task 9: Commit logging enhancements**
  - [ ] Commit message: `feat(logging): add comprehensive console logging for Epic 0 critical paths (Story 0.16.1)`
  - [ ] Push to branch, notify human engineer that Phase 1 complete
