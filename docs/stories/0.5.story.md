# Story 0.5: SKEL-API-002 - Auth Middleware (Basic)
...
- [x] Task 4: Update Bindings for Supabase Credentials (AC: 1)
  ...
  - [x] These should already exist from Story 0.4
  - [x] Source: [architecture/8-backend-architecture.md#8.2]
...
All auth errors follow the standard error format from Story 0.4:
...
2. `apps/api/src/types/bindings.ts` - Verify Supabase bindings exist (from Story 0.4)
3. `apps/api/wrangler.toml` - Add SUPABASE_URL to vars
4. `apps/api/README.md` - Update documentation

**Existing Files (verify):**
- `apps/api/src/types/bindings.ts` - Should have Supabase environment variables from Story 0.4
...
2. Continue to Story 0.6 or Epic 1 planning
