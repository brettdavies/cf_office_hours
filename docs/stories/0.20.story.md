...
- **Baked into the seed, refreshed on a schedule.** `scripts/convert_backup_to_d1.py` appends a self-correcting footer
  ...
  therefore correct with no manual step. The date portion (`scripts/bump-seed-dates.sql`) also runs weekly via the API
  Worker's Cron Trigger (the `scheduled` handler in `apps/api/src/index.ts`) to keep an already-loaded demo current.
