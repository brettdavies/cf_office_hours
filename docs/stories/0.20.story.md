...
- **Enforced as a post-seed correction.** The D1 seed is a static dump converted from the production backup, so the
  shape is applied after loading rather than during generation: `scripts/fix-seed-booking-rate.sql` thins bookings to a
  realistic per-mentee count and sets the status mix, and `scripts/reanchor-seed-dates.sql` slides all dates onto the
  current date and back-dates each request's `created_at` / `confirmed_at`. Run both after seeding (fix-booking-rate
  first).
